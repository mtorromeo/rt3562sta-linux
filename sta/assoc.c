	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv)) 
	{
		...
		if ((pAd->MlmeAux.HtCapabilityLen > 0) && (pAd->CommonCfg.PhyMode >= PHY_11ABGN_MIXED))
		{
			...
			PHT_CAPABILITY_IE pHtCapability = &pAd->MlmeAux.HtCapability;
#ifdef RT_BIG_ENDIAN
			...
			{
				NdisMoveMemory(&ht_cap, pHtCapability, pAd->MlmeAux.HtCapabilityLen);
				ht_cap.MCSSet[1] = 0x00;
				ht_cap.HtCapInfo.TxSTBC = 0;
				pHtCapability = &ht_cap;
			}

#ifdef RT_BIG_ENDIAN
			NdisZeroMemory(&HtCapabilityTmp, sizeof(HT_CAPABILITY_IE));
			NdisMoveMemory(&HtCapabilityTmp, pHtCapability, pAd->MlmeAux.HtCapabilityLen);
