...
//All done, it's time to register the net device to linux kernel.
...
#ifdef PRE_ASSIGN_MAC_ADDR
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("@%s MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n", 
									...
									PermanentAddress[4],
									PermanentAddress[5]));
		RtmpOSNetDevAddrSet(net_dev, &PermanentAddress[0]);
	}
	...
	if (!(IS_RT3090(pAd) || IS_RT3572(pAd) || IS_RT3390(pAd) || IS_RT3593(pAd)))
	{
				...
				else
		{
			...
			else
			{
				...
				if((pAd->NicConfig2.field.HwCoexEn == TRUE) &&
					((pAd->NicConfig3.field.CoexMethod == TRUE) || IS_ENABLE_SINGLE_CRYSTAL_SHARING_BY_FORCE(pAd)) &&
					(pAd->StaCfg.PSControl.field.rt30xxPowerMode == 0x3))
				{
					PCIePowerSaveLevel = 2;
					DBGPRINT(RT_DEBUG_TRACE,("====> rt30xx combo write 0x83 Comman = 0x%x.\n", PCIePowerSaveLevel));
				}
