...
//	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);
	...
	//		pAd->RalinkCounters.MgmtRingFullCount = 0;
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if ((MacReg & 0x20000000) && (MacReg & 0xA0))
			{
				RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL, 0x1);
				RTMPusecDelay(1);
				if (pAd->bHwCoexInit && (!IS_RT5390CB(pAd)) && (!IS_RT3592CB(pAd)))
				{
	...
	IN PUCHAR				pInitTxRateIdx)
{
	...
	//
	// RT3592BC8 TR0 shared with BT. If BT enabled, we can't use 2 stream (MAC can't large than MCS 7).
	//
	if (IS_RT3592CB(pAd) && (pAd->CommonCfg.Channel <= 14) && IS_WM_ON(pAd))
	{
		// fall back to 1x1 if necessary.
		if ((*ppTable == RateSwitchTable11N2S) || (*ppTable == RateSwitchTable11BGN2S))
		{
			*ppTable = RateSwitchTable11N1S;
			*pTableSize = RateSwitchTable11N1S[0];
			*pInitTxRateIdx = RateSwitchTable11N1S[1];
			DBGPRINT(RT_DEBUG_INFO, ("BC8: RateTable fall back to 1x1 \n"));
		}
	}
...
{
	INT 	  i, j;
	BSS_ENTRY *TmpBss = NULL;

	os_alloc_mem(NULL, (UCHAR **)&TmpBss, sizeof(BSS_ENTRY));
	if (TmpBss == NULL)
		return;
	...
	IN 	ADD_HT_INFO_IE			*pAddHtInfo)
{
	...
	if (IS_RT3592CB(pAd) && (pAd->MlmeAux.Channel <= 14) && IS_WM_ON(pAd))
	{
		/* Use 1x1 for RT3592BC8 on 2.4G */
		pAd->MlmeAux.HtCapability.MCSSet[0] = 0xff;
	}
	else
	{
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	
	{
		...
		if (IS_RT3592CB(pAd)) 
		{
			...
			if ((pAd->LatchRfRegs.Channel > 14) || (!IS_WM_ON(pAd)))
			{
				if(pAd->Antenna.field.RxPath == 3)
				{
					BBPR3 |= (0x10);
	...
	IN PRTMP_ADAPTER pAd)
{
		...
		)
	{
		...
		if (IS_RT3592CB(pAd) && (pAd->LatchRfRegs.Channel <= 14) && IS_WM_ON(pAd))
		{	
			Rssi = pAd->StaCfg.RssiSample.AvgRssi0;
		}
		else
