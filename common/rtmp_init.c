	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if ((IS_RT3592CB(pAd) && ((pAd->EepromVersion & 0x0f) >= 2))
			|| IS_ENABLE_WM_STATUS_CHECKING(pAd))
		{
			...
			if (FormalVersion >= 0x0201)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s():GPIO4 ACTIVE HIGH\n", __FUNCTION__));
			}
			WM_COEX_SET_FLAG(pAd, fWM_CHECK_RADIO);
 ...
 */
	...
	BBPR3 &= (~0x18);
	if (IS_RT3572(pAd) && (pAd->NicConfig2.field.HwCoexEn == TRUE))
	{
		BBPR3 &= (~0x3);
		BBPR3 |= 0x01;

		if ((INFRA_ON(pAd) || ADHOC_ON(pAd)) && 
			((pAd->CommonCfg.Channel > 14) || (!IS_WM_ON(pAd))))
		{
...
//	ULONG	i = 0;
	...
	if ((pAd->bHwCoexInit) && (!IS_RT3592CB(pAd)))
	{
